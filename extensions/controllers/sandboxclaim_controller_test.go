	...
	claim := &extensionsv1alpha1.SandboxClaim{
			...
			ObjectMeta: metav1.ObjectMeta{
				...
				OwnerReferences: []metav1.OwnerReference{

					{
						...
						APIVersion: "extensions.agents.x-k-s.io/v1alpha1",
		...
		testCases := []struct {
...

			{
				...
				name: "sandbox is created with metadata when a claim is made",
				...
				existingObjects: []client.Object{
					...
					&extensionsv1alpha1.SandboxTemplate{
						...
						ObjectMeta: metav1.ObjectMeta{
							...
							Name:      "test-template-with-metadata",
					...
					&extensionsv1alpha1.SandboxClaim{
						...
						ObjectMeta: metav1.ObjectMeta{
							...
							Name:      "test-claim-with-metadata",

							Namespace: "default",

							UID:       "claim-uid-with-metadata",
		...
		for _, tc := range testCases {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				// special case for the metadata test

				var req reconcile.Request

				var template extensionsv1alpha1.SandboxTemplate

				if tc.name == "sandbox is created with metadata when a claim is made" {

					client := fake.NewClientBuilder().WithScheme(scheme).WithObjects(tc.existingObjects...).WithStatusSubresource(&extensionsv1alpha1.SandboxClaim{}).Build()
					...
					if err := client.Get(context.Background(), types.NamespacedName{Name: "test-template-with-metadata", Namespace: "default"}, &template); err != nil {
					...
					if diff := cmp.Diff(sandbox.Spec.PodTemplate.ObjectMeta, template.Spec.PodTemplate.ObjectMeta); diff != "" {

						t.Errorf("unexpected sandbox metadata:\n%s", diff)
